...
// 左側に free slot があったら 1 、なかったら 0 を返す
int merge_left(my_metadata_t *base_metadata) {
  // look at the all free list. check if there is a free slot in the left of the base_metadata.
  // 左側に free slot があるか
  //    | left_metadata | free slot | base_metadata | free slot | ...
  //    ^                           ^
  //    left_metadata               base_metadata
  //                    <---------->
  //                    left_metadata->size
  ...
  if(!left_metadata) return 0;

  heap_idx = get_heap_idx(&base_metadata->size);
  // 右側の free slot をリストから消す
  my_remove_from_free_list(base_metadata, base_prev, &my_heaps[heap_idx]);

  if(left_prev == base_metadata) left_prev = NULL;
  heap_idx = get_heap_idx(&left_metadata->size);
  // left_metadata の size 更新に合わせて所属する free list を変更
  my_remove_from_free_list(left_metadata, left_prev, &my_heaps[heap_idx]);
  
  // left_metadata の size を更新
